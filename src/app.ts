...
const storeUrl = `https://api.bigcommerce.com/stores/${config.store_hash}/v3/catalog/products`;

const app: Express = express();
const emailRegExp = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
const passwRegExp = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&_])[A-Za-z\d@$!%*?&_]{8,}$/;
...
app.post('/login', (req, res) => {
  try {
    if (!(req.body && req.body.email && req.body.password)) return res.status(400).send('Email, Password required');
